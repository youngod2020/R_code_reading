...
##################################### day3 ###########################################
# 히트맵 작성 ------------------------------------------------------------------
# 너무 뻔한 데이터에 히트맵을 그리는 것은 비추함(소매유통업: 일간, 시간대별)
# bin수로 계급 조정, bin width 너비로 계급 조정
names(econ)

econ %>% 
    filter(key == "unemploy") %>% 
    mutate(year = str_sub(date, 1, 4), month = str_sub(date, 6, 7)) %>% 
    ggplot(aes(year, month, fill = values)) +
    geom_tile() +                                           # 히트맵
    theme(axis.text.x = element_text(angle = 90))           # x축 90도 돌리기
...
# 히스토그램의 계급: 계급구간이 좁은 경우, 중앙을 알기 어려움
# 1. 시행 착오로 너비 조정
# 2. 히스토그램에 대한 적절한 너비를 구하는 공식이 있음

# 상관계수 그래프----------------------------------------------------------------
data(mtcars)
library(tidymodels) # tidy를 사용하려면 import되어야 함
mtcars %>%
    select(-mpg) %>%
    map(~ cor.test(.x, mtcars$mpg)) %>%
    map(tidy) %>%
    bind_rows(.id = "variable") %>%
    ggplot(aes(reorder(variable, estimate), estimate)) +
    geom_point() +
    geom_errorbar(aes(ymin = conf.low, ymax = conf.high), width = 0.1) +
    labs(x = "variables", y = "coef of correlation with mpag")
...
mtcars %>% 
    select_if(is.numeric) %>% 
    correlate() %>% 
    rearrange(method = "PCA", absolute = F) %>% 
    network_plot()
...
## animate data II
q <- ggplot(gapminder, aes(gdpPercap, lifeExp, size = pop, colour = country)) +
